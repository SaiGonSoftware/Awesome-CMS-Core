...
type EditUserRolesModalState = {
  rolesName: any | undefined[],
  currentUserRoles: any | undefined[],
  userId: any | string,
  loading: boolean
};
...
> {
  ...
  editUserRole = e => {
    ...
    const isRoleValid = this.state.currentUserRoles.length > 0;
    if (isRoleValid) {
      PutWithSpinner.call(this, USER_ROLES_EDIT_API, {
  ...
  handleRoleSelection = e => {
    ...
    if (this.state.currentUserRoles.indexOf(newSelection) > -1) {
      newSelectionArray = this.state.currentUserRoles.filter(
        s => s !== newSelection
      );
    } else {
