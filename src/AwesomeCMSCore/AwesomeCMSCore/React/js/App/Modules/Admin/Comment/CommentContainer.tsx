...
class CommentContainer extends Component<{}, CommentContainerState> {
    ...
    switch (commentStatus) {
      case CommentStatus.Pending:
        ...
        if (this.state.comments.pendingComments.includes(commentToRemove)) {
          ...
          btnSelector.classList.add("btn-outline-success-active");
          Put(
            `${COMMENTS_ENDPOINT}/comment/${commentId}/${
      ...
      case CommentStatus.Approved:
        ...
        if (this.state.comments.pendingComments.includes(commentToRemove)) {
          ...
          btnSelector = document.getElementById(`allComments-${commentId}`);
          btnSelector.classList.remove("btn-outline-success-active");
          btnSelector = document.getElementById(
            `approvedComments-${commentId}`
          );
          btnSelector.classList.remove("btn-outline-success-active");
          Put(
            `${COMMENTS_ENDPOINT}/comment/${commentId}/${CommentStatus.Pending}`
          ).then(res => {
            if (res.status === StatusCode.Success) {
              toastr.info("Edit comment status success");
              this.setState({ comments: updatedComments });
      ...
      case CommentStatus.Spam:
        commentToRemove = this.state.comments.spamComments.find(
          cm => cm.comment.id == commentId
        );
        if (this.state.comments.spamComments.includes(commentToRemove)) {
          updatedComments.spamComments.splice(
            this.state.comments.spamComments.indexOf(commentToRemove),
            1
          );
          updatedComments.approvedComments.push(commentToRemove);
          ...
          Put(
            ...
            if (res.status === StatusCode.Success) {
              ...
              for (let item of btnSelector) {
                item.classList.remove("spam-actions-hidden");
              }
            }
          });
        }
      ...
      case CommentStatus.Trash:
        commentToRemove = this.state.comments.deletedComments.find(
          cm => cm.comment.id == commentId
        );
        if (this.state.comments.deletedComments.includes(commentToRemove)) {
          updatedComments.deletedComments.splice(
            this.state.comments.deletedComments.indexOf(commentToRemove),
            1
          );
          updatedComments.approvedComments.push(commentToRemove);
          ...
          Put(
            ...
            if (res.status === StatusCode.Success) {
              ...
              btnSelector = document
                ...
                .getElementsByTagName("button");
              for (let item of btnSelector) {
                item.classList.remove("trash-actions-hidden");
              }
            }
          });
        }
  ...
  toggleSpamComment = (commentStatus, commentId) => {
    ...
    switch (commentStatus) {
      case CommentStatus.Approved:
        commentToRemove = this.state.comments.allComments.find(
          cm => cm.comment.id == commentId
        );
        updatedComments.allComments.splice(
          this.state.comments.allComments.indexOf(commentToRemove),
          1
        );
        updatedComments.approvedComments.splice(
          this.state.comments.approvedComments.indexOf(commentToRemove),
          1
        );
        updatedComments.spamComments.push(commentToRemove);
        updatedComments.numberOfApprovedComments > 0
          ? (updatedComments.numberOfApprovedComments -= 1)
          : updatedComments.numberOfApprovedComments;
        updatedComments.numberOfSpamComments += 1;
        Put(
          `${COMMENTS_ENDPOINT}/comment/${commentId}/${CommentStatus.Spam}`
        ).then(res => {
          if (res.status === StatusCode.Success) {
            toastr.info("Edit comment status success");
            this.setState({ comments: updatedComments });
            ...
            for (let i = 0; i < btnSelector.length; i++) {
              if (i != 0 && i != 2) {
                btnSelector[i].classList.add("spam-actions-hidden");
              }
            }
  ...
  toggleDeleteComment = (commentStatus, commentId) => {
    ...
    switch (commentStatus) {
      case CommentStatus.Pending:
        ...
        updatedComments.deletedComments.push(commentToRemove);
        updatedComments.numberOfPendingComments > 0
          ? (updatedComments.numberOfPendingComments -= 1)
          : updatedComments.numberOfPendingComments;
        updatedComments.numberOfDeletedComments += 1;
        Put(
          `${COMMENTS_ENDPOINT}/comment/${commentId}/${CommentStatus.Trash}`
        ).then(res => {
          if (res.status === StatusCode.Success) {
            toastr.info("Edit comment status success");
            this.setState({ comments: updatedComments });
            ...
            for (let i = 0; i < btnSelector.length; i++) {
              if (i != 0) {
                btnSelector[i].classList.add("trash-actions-hidden");
              }
            }
          }
        });
  ...
  toggleReplyBox = e => {
    ...
    if (this.state.opened.indexOf(id) != -1) {
      // remove from array
      this.setState({
        opened: this.state.opened.filter(o => o !== id)
      });
    } else {
      this.setState({
  ...
  onReply = (comment, e) => {
    if (e.charCode === 13) {
      const replyViewModel = {
        Comment:
          comment.parentComment !== null ? comment.parentComment : comment,
        CommentBody: e.target.value
      };
      const url = `${COMMENTS_ENDPOINT}/${comment.id}/reply`;
      PostWithSpinner.call(this, url, replyViewModel)
        .then(res => {
          if (res.status === StatusCode.Success)
